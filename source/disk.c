...
u8 diskCmdSD(u8 cmd, u32 arg, u8* sd_resp_buff) {
...
u8 diskInit() {
    ...
    resp = diskCmdSD(CMD0, 0x1aa, sd_resp_buff);

    for (i = 0; i < 40; i++)bi_sd_dat_wr(0xff);

    resp = diskCmdSD(CMD8, 0x1aa, sd_resp_buff);
    ...
    if (card_type == SD_V2) {
        ...
        for (i = 0; i < wait_len; i++) {
            ...
            resp = diskCmdSD(CMD55, 0, sd_resp_buff);
            if (resp)return DISK_ERR_INIT + 1;
            if ((sd_resp_buff[3] & 1) != 1)continue;
            resp = diskCmdSD(CMD41, 0x40300000, sd_resp_buff);
    ...
    resp = diskCmdSD(CMD2, 0, sd_resp_buff);
    if (resp)return DISK_ERR_INIT + 5;

    resp = diskCmdSD(CMD3, 0, sd_resp_buff);
    if (resp)return DISK_ERR_INIT + 6;

    resp = diskCmdSD(CMD7, 0, sd_resp_buff);


    rca = (sd_resp_buff[1] << 24) | (sd_resp_buff[2] << 16) | (sd_resp_buff[3] << 8) | (sd_resp_buff[4] << 0);


    resp = diskCmdSD(CMD9, rca, sd_resp_buff); //get csd
    if (resp)return DISK_ERR_INIT + 7;


    resp = diskCmdSD(CMD7, rca, sd_resp_buff);
    if (resp)return DISK_ERR_INIT + 8;


    resp = diskCmdSD(CMD55, rca, sd_resp_buff);
    if (resp)return DISK_ERR_INIT + 9;


    resp = diskCmdSD(CMD6, 2, sd_resp_buff);
...
u8 diskOpenRead(u32 saddr) {
    ...
    resp = diskCmdSD(CMD18, saddr, sd_resp_buff);
...
u8 diskCloseRW() {
    ...
    disk_addr = ~0;
    resp = diskCmdSD(CMD12, 0, sd_resp_buff);
...
void diskPowerDown() {
    u8 sd_resp_buff[18];
    diskCmdSD(CMD0, 0x1aa, sd_resp_buff);
}
...
u8 diskWrite(u32 sd_addr, void *src, u16 slen) {
    ...
    resp = diskCmdSD(CMD25, saddr, sd_resp_buff);
